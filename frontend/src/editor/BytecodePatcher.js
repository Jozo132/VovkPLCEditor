...
export default class BytecodePatcher {
    #editor = null
    ...
    async _readIREntries() {
        ...
        if (!runtime) {
            console.warn('[BytecodePatcher] No runtime available')
            return []
        }
    ...
    async scanPatchableConstants() {
        ...
        for (const entry of irEntries) {
            ...
            for (let opIdx = 0; opIdx < entry.operands.length; opIdx++) {
                ...
                const operandBytecodeOffset = entry.bytecode_offset + operand.bytecode_pos
                ...
                if (entry.flags & IR_FLAG_TIMER) {
                    // Timer instruction - find timer address from first operand
                    instructionName = OPCODE_NAMES[entry.opcode] || 'TIMER'
                    if (entry.operands.length >= 1 && entry.operands[0].type === IR_OP_PTR) {
                        timerAddress = entry.operands[0].value
        ...
        console.log(`[BytecodePatcher] Found ${patches.length} patchable constant(s)`)
