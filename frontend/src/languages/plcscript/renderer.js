                ...
                if (editor.context_manager) {
                    editor.context_manager.show(e, items, async () => {
                        try {
                            ...
                            // Try specific compilePLCScript method, fallback to compile with language option
                            let result
                            if (typeof editor.runtime.compilePLCScript === 'function') {
                                result = await editor.runtime.compilePLCScript(block.code)
                            } else if (typeof editor.runtime.compile === 'function') {
                                result = await editor.runtime.compile(block.code, {language: 'plcscript'})
                            } else {
